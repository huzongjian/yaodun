...
<div style="display: none;">
	<table>
		<tr id="useToInsert">
			...
			<td style="display: none;"><input type="text" name="usage" id="usage"
				placeholder="" maxlength="40" class="input-medium"
				style="width: 95%" /></td>
			<td style="display: none;"><input type="text" name="dosage" id="dosage"
				placeholder="" maxlength="40" class="input-control"
				style="width: 65%" /><span class="input-group-addon"  id="unitinit" ></span></td>
			<td style="display: none;"><input type="text" name="frequery" id="frequery"
				placeholder="" maxlength="40" class="input-medium"
				style="width: 95%" /></td>
			<td width="13%" style="text-align: center;"><a
...
<div class="navbar navbar-fixed-top">
	...
	<div class="navbar-inner">
		...
		<div class="container">
			...
			</a> <a class="brand" href="./">康众用药安全卫士</a>
			...
			<div class="nav-collapse">
			 ...
			 <ul class="nav pull-right">
					...
					<li class="dropdown"><a data-toggle="dropdown"  
						class="dropdown-toggle " href="#" style="color: black; "> ${user.userName} <b class="caret" style="color: black;"></b>
					</a>

						<ul class="dropdown-menu">
						<li><a href="/yaodun/Knowledge/load"><i class="icon-user"></i>
									文章</a></li>
							<li><a href="/yaodun/User/account"><i class="icon-user"></i>
									注册</a></li>

							<li><a href="/yaodun/User/loginOut"><i
									class="icon-lock"></i>登陆</a></li>
						...
						<div id="collapseOne" class="accordion-body collapse"
							...
							<div class="accordion-inner" style="border: 2px">
								<div class="control-group">
									<div class="controls">
										<label for="control-label">性别</label> <select style="width:190px" id="male">
								...
								<div class="control-group">
									<div class="controls">
										<label for="control-label">妊娠期</label> <select style="width:190px" id="gestate">
								...
								<div class="control-group">
									<div class="controls">
										<label for="control-label">哺乳期</label> <select style="width:190px" id="suckling">
												<option value="0">是</option>
											<option value="1"  selected="selected">否</option>
										</select>
									</div>
			...
			<div class="span9">
						...
						<table class="table table-striped table-bordered" id="drugtable">
							<thead>
								<tr>
									...
									<th style="display: none;">用法</th>
									<th style="display: none;">用量</th>
							...
							<tbody id="druginfo">
								<tr>
									...
									<td style="display: none;"><input type="text" name="usage" id="usageinit"
										placeholder="" maxlength="40"
										class="input-medium" style="width: 95%" /></td>
									<td style="display: none;"><input type="text" name="dosage" id="dosageinit"
										placeholder="" maxlength="40"
										class="input-control" style="width: 65%" />
										<span class="input-group-addon" id="unitinit" ></span>
										</td>
									<td style="display: none;"><input type="text" name="frequery" id="frequeryinit"
										placeholder="" maxlength="40"
										class="input-medium" style="width: 95%" /></td>
									<td  width="13%" style="text-align: center;"><a
										class="btn btn-small btn-warning icon-plus"> </a> <a
				...
				<div class="row">
					...
					<div class="widget" >
							...
							<div class="widget-content " id="checkresult">
				...
				<div class="row">
					...
					<div class="span4">
						...
						<div class="widget" >
							...
							<div class="widget-content " >
								...
								<p>  如：商品名  通用名    商品名  通用名     
								<p>  新达罗：头孢克洛   护彤：小儿氨酚黄那敏     
								<p>  希刻劳：头孢克洛   库克：小儿氨酚黄那敏
								<p>  类似这些同一药物不同商品名的药品建议不要同时使用
							</div>
							<!-- /widget-content -->
					...
					<div class="span4">
						...
						<div class="widget" >
							...
							<div class="widget-header" style="background-color: rgb(0, 191, 255) " >
								<h3 >不同药物因复方制剂同时使用</h3>
							</div>
							<!-- /widget-header -->

							<div class="widget-content " >								
								<p>如：感康，快克，三九感冒灵胶囊（颗粒）、维C银翘片
								<p>成分中均含有对乙酰氨基酚 
								<p>类似这些不同药物因含相同复方制剂的药品建议不要同时使用
					...
					<div class="span4">
						...
						<div class="widget">
							...
							<div class="widget-content" >
								...
								<p>  如：氧氟沙星和诺氟沙星同时使用，同属喹诺酮类抗生素
								<p>  氨氯地平与硝苯地平同时使用，同属钙离子抗拮剂
                                 <p> 类似这些同类药物或同类功效中成药的药品建议不要同时使用
	...
	$("#check").click(function () {
		...
		$("#drugtable tr:not(:first)").each(
			...
			$("input,select",this).each(function () {
				if (typeof($(this).attr("real-value")) == "undefined" ||$(this).attr("real-value")=="") {
	...
	function modelshow(datas) {
		$.ajax({
			...
			{ // 在这里拼接数据并组装成表格
				...
				var talbe ="<table class='table table-striped table-bordered' id='resulttable'><thead><tbody>";
				...
				var middle ="</td><td>";
				var end ="</td></tr>";
				var advicestart="<div class='widget-content'><h4 style='color: red'>药师建议：</h4>";
				var adviceend="</div>";
				var advice="";
				//把提示级别相同的数据放一起
				...
				if(advice!=""){
					advice= advicestart + advice+adviceend;
				}
				checkresult =warn+caution+ban;
				checkresult =talbe+checkresult+"</tbody></table>";
				
				$("#result").html(checkresult+advice);
				if (msg.length > 0) {
				//	$('#checkResult').modal('show');
					$("#checkresult").html(checkresult+advice);
				}
	...
	function drugInitAutocomplete(id) {
		...
		.autocomplete({
			...
			formatItem : function (item) {
				...
				//return "("+ item["drugname"]+ "，"+ item["productName"]+ ") ";
			},
